...
def freeze(network, freeze_layer):
    for name, param in network.named_parameters():
        with open('temp_log.txt', 'a') as f:
            print(f'name: {name}, param: {param.shape}', file=f)
        if freeze_layer not in name:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--freeze-layer', default='fusion_cbam', type=str, choices=['fusion_cbam','fusion_shuffle','fusion_eca','fusion_mrf','fusion_sknet'])
    ...
    # for p in training_bench.parameters():
    #     if p.requires_grad:
    #         print(f"{p.numel} is trainable.")
    #     else:
    #         print(f"{p.numel} is frozen.")
